...
BOOST_FIXTURE_TEST_SUITE(Car_Controller, CarContollerFixture)
	...
	{
		VerifyCommandHandling("EngineOn", "Car had turned on\n");
	}
	...
	BOOST_FIXTURE_TEST_SUITE(when_car_is_turn_on, when_car_turned_on_)
		...
		{
			VerifyCommandHandling("SetGear 1", "Condition of gear box had changed\n");
		}
		...
		{
			VerifyCommandHandling("SetSpeed 10", "Speed had not changed\n");
		}
		BOOST_AUTO_TEST_CASE(can_turn_off_car_when_car_is_standing_still)
		{
			VerifyCommandHandling("EngineOff", "Car had turned off\n");
		}
		BOOST_AUTO_TEST_CASE(cannot_turn_off_car_on_while_driving)
		{
			VerifyCommandHandling("SetGear -1", "Condition of gear box had changed\n");
			VerifyCommandHandling("SetSpeed 10", "Speed had changed\n");
			VerifyCommandHandling("Info", "Car is turned on\nDirection is -1\nSpeed is 10\nCondition of gear box is -1\n");
			VerifyCommandHandling("EngineOff", "Car had not turned off\n");
		}
	...
	BOOST_AUTO_TEST_SUITE(when_enigne_is_turn_off)
		...
		{
			VerifyCommandHandling("SetGear 1", "Car had not turned on\n");
		}
		BOOST_AUTO_TEST_CASE(cannot_change_speed)
		{
			VerifyCommandHandling("SetSpeed 10", "Car had not turned on\n");
		}
